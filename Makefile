...
DOCKER_IMAGE_REPO ?= travisci/travis-live
...
.PHONY: docker-build
docker-build:
	$(DOCKER) build --build-arg bundle_gems__contribsys__com=$(BUNDLE_GEMS__CONTRIBSYS__COM) -t $(DOCKER_DEST) . -f Dockerfile.tcie
